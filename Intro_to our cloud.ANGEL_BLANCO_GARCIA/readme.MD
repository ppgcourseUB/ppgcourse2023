...
* Please don't use the cluster for running tasks that can be run easily or more conveniently on your regular computer. A cluster is not a substitute for your computer, it's a computing dedicated system.  
...
* Command line access: Needed for launching jobs on the cluster. Use any relatively recent ssh client:
    ...
    - Windows:
        - Using WLS (Windows Linux Subsystem): open the command line application an type the previous ssh command)
...
* File transfer:
    - Graphical application:
        - FileZilla client is one of the easiest to install and run, and works on Linux, Mac and Windows (it's free too). 
    - Command line:
...
After accessing the system, we will be submitting our computing tasks to a specific software that manages the cluster, schedules our tasks to be run on specific nodes according to several criteria (like the resources available on the computing nodes, permissions, priorities, etc), supervises its execution on the allocated nodes once there are enough available resources, and manages its termination (either succesful or not). In our case we will be using SLURM, and the computing tasks that we submit to it are called jobs. 

Before submiting our jobs to SLURM, there may be previous steps for setting up the data we will use and the environment of the software we need to run in the jobs. Some of them are quite common, like loading software modules if our cluster supports it (although they are not mandatory and will depend on any specific case).

SLURM is a powerful software that gives users and administrators a lot capabilities and options, so there are many ways of setting up and controlling our jobs in SLURM and several of them can get rather complex. However, since in our particular case the goal is to exemplify the process in a relatively strightforward fashion, we will focus on a simple process, that we descibe next:

### Getting the data for our job:

The needed data for jyour job may be downloaded from another internet site, or uploaded from your personal computer, or it may bealready be present in a sepcific location of the cluster. Your instructor will specify how to get or reach the data you will need for each of your jobs. 

### Loading software modules

Modules are a tool to automate configuration of your environment for running a specific program. Keep in mid that in clusters and other shared systems, the amount of software installed can increase quickly and each software may need specific environment variables to run correctly. The Modules tool makes it simpler for the user and the administrator by storing and automating these settings.
...
You can use 'module list' to get the modules yu already have loaded:
...
Sometimes you would need to unload all your loaded modules to ensure there are no module incompatibilities before loading a specific module. You can unload all your module with 'module purge'
...
Some relevant bits to highlight:
...
* '--job-name': SLURM option to set a name for the job, so you can identify more easily
* '--mem': SLURM option to allocate an amout of RAM memory for the job.
* '-o' : File for the the job's output messages. Not that % translates to the number identifier of the job, which is an integer that will be showed to you when you submit the job
* '-e': File for the job's error messages (if any). Note that % is used again.

Next, you have other commands that may be needed to run correctly the program specified below. In this case we load the modules needed directly from this script. For instance, We could also set other needed environment variables, create directories for the results (mkdir ...), create files with some content for input or configuration of the program, remove temporary files that could have been left from other runs...

And finally, we have the actual command we want to run, in this case:

```
orthofinder -f proteomes -og -t 8
```

Note that the specific parameter of the program to specify the number of execution threads to run (in this case -t), matches the value of the SLURM option '-c'.

This is just a simple example, but there are many other SLURM options that can be set to modify the job submission, several of them depend on the kind of the program to be run (what kind of parallelization it supports for example). Again, your instructors will explain other relevant aspects about SLURM batch files, on a case by case basis.


### Submiting our job:

To submit a job based on a batch file, the 'sbatch' command is used followeb by the name (with path if needed) of the run script:
...
### Checking your submitthed jobs

One the job is submitted, you can check its state in several ways. Just 2 examples:
...
Note that the state of the jobs ins these cases is 'R', which means running. If the sate is 'W', it would mean that there are no resources available on any node to run the job. SLURM will keep it waiting until there are resources available, then run it.

You can chec the state of the computing nodes with a query like this for exampl:
